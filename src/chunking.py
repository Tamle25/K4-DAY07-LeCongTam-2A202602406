...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split on sentence boundaries while preserving the sentence text
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # Fallback when no separators left: hard cut by chunk_size
        ...
        for piece in splits:
            if not piece and sep != "":
                continue
            if len(piece) > self.chunk_size:
                sub_pieces = self._split(piece, next_separators)
                final_pieces.extend(sub_pieces)
            else:
                final_pieces.append(piece)

        # Merge adjacent small pieces up to chunk_size
        ...
        for piece in final_pieces:
            if not curr_acc:
                ...
                candidate = curr_acc + sep + piece
                if len(candidate) <= self.chunk_size:
                    curr_acc = candidate
                else:
                    merged_chunks.append(curr_acc)
                    curr_acc = piece
